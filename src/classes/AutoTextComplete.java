...
public class AutoTextComplete extends JWindow implements KeyListener, FocusListener, ActionListener {

    JTextComponent parent = null;
    JTextField field, name, Daddress = null;
    JList lst = null;
    ...
    public AutoTextComplete() {
        ...
        setButtons();
        ((JPanel) this.getContentPane()).setBorder(BorderFactory.createLineBorder(Color.white, 3));
    ...
    private void select(boolean enterPressed) {
        ...
        } else //amo = String.valueOf(lst.getSelectedValue()).split("##");
        {
    ...
    private void tableSelection() {
        ListSelectionModel sel = tableParent.getSelectionModel();
        ...
        // int row = tableParent.getRowCount(),i=0;
        // while(i<row){
        //        if(tableParent.getValueAt(row, 0).equals(value)){
        //           sel.setSelectionInterval(row, 0);
        //           return;
        //        }
        //  i++;
        // }
    ...
    public void focusGained(FocusEvent fe) {
        if (fe.getSource() == lst) {
            parent.requestFocus();
        } else if (fe.getSource() == parent || (fe.getSource() == tableParent && tableParent.getSelectedColumn() == activeColumn)) {
            populateList();
        }
    }
    ...
    public void keyPressed(KeyEvent ke) {
        ...
        if (kc == KeyEvent.VK_F5 || (parent instanceof JTextField && kc == KeyEvent.VK_UP)) {
            moveUp();
        } else if (kc == KeyEvent.VK_F5 || (tableParent instanceof JTable && kc == KeyEvent.VK_UP)) {
            moveUp();
        } else if (kc == KeyEvent.VK_F6 || (parent instanceof JTextField && kc == KeyEvent.VK_DOWN)) {
            moveDown();
        } else if (kc == KeyEvent.VK_F6 || (tableParent instanceof JTable && kc == KeyEvent.VK_DOWN)) {
            moveDown();
        }
    }
    ...
    private void populateList() {
        ...
        if (p.y - this.getHeight() > 0) {
            this.setLocation(p.x, p.y - (this.getHeight() + 6));
